...
#pragma mark - WMXYRouter_private
@interface NSString (WMXYRouter_private)
- (NSMutableDictionary *)__uxy_dictionaryFromQueryComponents;
@end

@interface WMXYRouterNib : NSObject
@property (nonatomic, copy) NSString *nibName;
@property (nonatomic, strong) NSBundle *bundle;
@end

@implementation WMXYRouterNib
@end
...
@implementation WMXYRouter

+ (instancetype)sharedInstance
{
    static WMXYRouter *router = nil;
...
- (void)mapKey:(NSString *)key toNibName:(NSString *)nibName bundle:(NSBundle *)bundle
{
    ...
    WMXYRouterNib *nib = [[WMXYRouterNib alloc] init];
    nib.nibName = nibName;
    nib.bundle  = bundle;
    _map[key]   = nib;
}
...
- (id)viewControllerForKey:(NSString *)key
{
    ...
    else if ([obj isKindOfClass:[WMXYRouterNib class]])
    {
        vc = [[UIViewController alloc] initWithNibName:((WMXYRouterNib *)obj).nibName
                                                bundle:((WMXYRouterNib *)obj).bundle];
    }
    else
    {
        WMXYRouterBlock objBlock = (WMXYRouterBlock)obj;
        vc = objBlock();
    }
...
@implementation UIViewController (WMXYRouter)

static const char *WMXYRouter_URLPath = "WMXY.ViewController.URLPath";
- (NSString *)uxy_URLPath
{
    return objc_getAssociatedObject(self, WMXYRouter_URLPath);
}

- (void)setUxy_URLPath:(NSString *)uxy_URLPath
{
    objc_setAssociatedObject(self, WMXYRouter_URLPath, uxy_URLPath, OBJC_ASSOCIATION_COPY_NONATOMIC);
}
